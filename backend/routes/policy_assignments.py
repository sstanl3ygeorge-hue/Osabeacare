...
async def export_policy_acknowledgement_pdf(
    ...
    # Import branded utilities from pdf_service
    ...
    styles['FormTitle'] = ParagraphStyle(
        name='FormTitle',
        parent=base_styles['Heading1'],
        fontSize=18,
    ...
    # Title
    elements.append(Paragraph("Policy Acknowledgement Record", styles['FormTitle']))
    
    # Policy details in branded table
    ack_data = [
        ...
        ['Acknowledged At', str(assignment.get('acknowledged_at') or '-')],
        ['Assignment Status', assignment.get('status') or '-'],
        ['Admin Reviewed', 'Yes' if assignment.get('admin_reviewed') else 'No'],
        ['Admin Reviewed At', str(assignment.get('admin_reviewed_at') or '-')],
    ]
    
    ack_table = Table(ack_data, colWidths=[50*mm, 120*mm])
