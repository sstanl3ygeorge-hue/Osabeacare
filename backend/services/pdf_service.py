...
def generate_evidence_record_pdf(
    document_title: str,
    employee_data: Dict[str, Any],
    evidence_details: Dict[str, Any],
) -> bytes:
    """Generate a branded PDF evidence record for internal training or induction evidence."""
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(
        buffer,
        pagesize=A4,
    ...
    if logo:
        header_data.append([logo, ""])
    header_data.append([Paragraph("Osabea Healthcare Solutions", styles['CompanyName']), ""])
    header_data.append([Paragraph("Compliance Evidence Record", styles['Subtitle']), ""])
    ...
    elements.append(Spacer(1, 5*mm))
    elements.append(HRFlowable(width="100%", thickness=1, color=BORDER_COLOR, spaceAfter=5*mm))
    elements.append(Paragraph(document_title, styles['FormTitle']))

    employee_name = f"{employee_data.get('first_name', '')} {employee_data.get('last_name', '')}".strip() or employee_data.get('name', 'Unknown')
    employee_code = employee_data.get('employee_code') or employee_data.get('applicant_reference') or employee_data.get('id', '')[:8]
    generated_at = evidence_details.get('generated_at') or datetime.now(timezone.utc).strftime('%d %B %Y %H:%M UTC')

    info_data = [
        ['Employee:', employee_name],
        ['Employee ID:', employee_code or 'N/A'],
        ['Generated:', generated_at],
        ['Record reference:', evidence_details.get('record_reference') or 'N/A'],
    ]
    info_table = Table(info_data, colWidths=[35*mm, 135*mm])
    info_table.setStyle(TableStyle([
        ('FONTNAME', (0, 0), (0, -1), 'Helvetica-Bold'),
    ...
    detail_rows = [
        ['Item', evidence_details.get('item_name') or 'N/A'],
        ['Evidence type', evidence_details.get('evidence_type') or 'N/A'],
        ['Source', evidence_details.get('source_label') or 'Internal record'],
        ['Completed date', evidence_details.get('completed_at') or 'N/A'],
        [evidence_details.get('reviewed_label') or 'Verified by', evidence_details.get('reviewed_by') or 'N/A'],
    ]
    if evidence_details.get('reviewed_at'):
        detail_rows.append(['Reviewed date', evidence_details.get('reviewed_at')])
    if evidence_details.get('expiry_date'):
        detail_rows.append(['Expiry date', evidence_details.get('expiry_date')])

    details_table = Table(detail_rows, colWidths=[45*mm, 125*mm])
    ...
    notes = [note for note in (evidence_details.get('notes') or []) if note]
    if notes:
        elements.append(Paragraph('Evidence Notes', styles['SectionHeader']))
        for note in notes:
            elements.append(Paragraph(escape(str(note)), styles['Notes']))

    elements.append(Spacer(1, 10*mm))
    elements.append(HRFlowable(width="100%", thickness=0.5, color=BORDER_COLOR, spaceBefore=5*mm))
    elements.append(Paragraph(
        f"Generated by Osabea Healthcare Solutions Compliance System on {datetime.now(timezone.utc).strftime('%d %B %Y %H:%M UTC')}",
        styles['Footer'],
    ))
    elements.append(Paragraph("This is an official compliance evidence record.", styles['Footer']))
